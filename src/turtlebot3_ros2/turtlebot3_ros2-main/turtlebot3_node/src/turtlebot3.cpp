...
void TurtleBot3::init_dynamixel_sdk_wrapper(const std::string &usb_port) {
  ...
  this->get_parameter_or<float>("opencr.protocol_version",
                                opencr.protocol_version, 2.0f);
  ...
  robit_dxl_sdk_wrapper_ = std::make_shared<DynamixelSDKWrapper>(opencr);

  robit_dxl_sdk_wrapper_->init_read_memory(
      extern_control_table.robit_button.addr,
      (extern_control_table.robit_psd_r.addr -
       extern_control_table.robit_button.addr) +
          extern_control_table.robit_psd_r.length);
}
...
void TurtleBot3::check_device_status() {
  if (dxl_sdk_wrapper_->is_connected_to_device()) {
    ...
    dxl_sdk_wrapper_->set_data_to_device(
        ...
        extern_control_table.imu_re_calibration.length, &reset, &sdk_msg);
  ...
  case NOT_CONNECTED_MOTOR:
    RCLCPP_WARN(this->get_logger(),
                "Please double check your Dynamixels and Power");
    break;
...
void TurtleBot3::add_motors() {
  ...
  this->get_parameter_or<float>("motors.profile_acceleration_constant",
                                motors_.profile_acceleration_constant, 214.577);

  this->get_parameter_or<float>("motors.profile_acceleration",
                                motors_.profile_acceleration, 0.0);
}
...
void TurtleBot3::add_sensors() {
  ...
  this->get_parameter_or<uint8_t>("sensors.illumination",
                                  is_connected_illumination, 1);
  ...
  sensors_.push_back(
      new sensors::Imu(node_handle_, "imu", "magnetic_field", "imu_link"));

  sensors_.push_back(new sensors::SensorState(
      node_handle_, "sensor_state", is_connected_bumper_1,
      is_connected_bumper_2, is_connected_illumination, is_connected_ir,
      is_connected_sonar));

  sensors_.push_back(
      new sensors::JointState(node_handle_, "joint_states", "base_link"));

  robit_sensors_.push_back(new sensors::RobitButton(node_handle_));
  robit_sensors_.push_back(new sensors::RobitPSD(node_handle_));
}
...
void TurtleBot3::add_devices() {
  ...
  mode_led_ = new devices::RobitLED(node_handle_, dxl_sdk_wrapper_);
}
...
void TurtleBot3::publish_timer(const std::chrono::milliseconds timeout) {
  publish_timer_ = this->create_wall_timer(timeout, [this]() -> void {
    ...
    robit_dxl_sdk_wrapper_->read_data_set();
    for (const auto &sensor : robit_sensors_) {
      sensor->publish(now, robit_dxl_sdk_wrapper_);
    }
  });
}
...
void TurtleBot3::heartbeat_timer(const std::chrono::milliseconds timeout) {
  heartbeat_timer_ = this->create_wall_timer(timeout, [this]() -> void {
    ...
    dxl_sdk_wrapper_->set_data_to_device(extern_control_table.heartbeat.addr,
                                         extern_control_table.heartbeat.length,
                                         &count, &msg);

    RCLCPP_DEBUG(this->get_logger(), "hearbeat count : %d, msg : %s", count,
                 msg.c_str());
...
void TurtleBot3::parameter_event_callback() {
  ...
  auto param_event_callback =
    ...
    for (const auto &changed_parameter : event->changed_parameters) {
      RCLCPP_DEBUG(this->get_logger(), "changed parameter name : %s",
      ...
      if (changed_parameter.name == "motors.profile_acceleration") {
        ...
        motors_.profile_acceleration =
            rclcpp::Parameter::from_parameter_msg(changed_parameter)
                .as_double();

        motors_.profile_acceleration = motors_.profile_acceleration /
        ...
        dxl_sdk_wrapper_->set_data_to_device(start_addr, addr_length, p_data,
                                             &sdk_msg);

        RCLCPP_INFO(this->get_logger(),
                    "changed parameter value : %f [rev/min2] sdk_msg : %s",
                    motors_.profile_acceleration, sdk_msg.c_str());
      }
    }
  };

  parameter_event_sub_ =
      priv_parameters_client_->on_parameter_event(param_event_callback);
}
...
void TurtleBot3::cmd_vel_callback() {
  ...
  cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
      ...
      [this](const geometry_msgs::msg::Twist::SharedPtr msg) -> void {
        ...
        RCLCPP_DEBUG(this->get_logger(), "lin_vel: %f ang_vel: %f msg : %s",
